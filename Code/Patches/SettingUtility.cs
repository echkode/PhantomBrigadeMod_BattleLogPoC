...
namespace EchKode.PBMods.BattleLog
{
	static class SettingUtility
	{
		...
		{
			if (ModLink.Settings.IsLoggingEnabled(ModLink.ModSettings.LoggingFlag.KeyBinding))
			{
				Debug.LogFormat(
		...
		private static void RefreshPlayerForInputHelper()
		{
			...
			{
				Debug.LogFormat(
					"Mod {0} ({1}) replaced player on InputHelper",
					ModLink.modIndex,
					ModLink.modId);
			}
		}
		...
		private static void RefreshPlayerForCIViews()
		{
			...
			{
				var components = rgo.GetComponentsInChildren<CIView>(true);
				if (components == null)
				{
					continue;
				}
				...
				foreach (var c in components)
				{
					...
					{
						Debug.LogFormat(
							"Mod {0} ({1}) replaced player | component : {2} | game object: {2}",
							ModLink.modIndex,
							ModLink.modId,
							c.GetType().Name,
							rgo.name);
